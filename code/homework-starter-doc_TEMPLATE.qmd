...
1. Total mass is positively correlated the mass of roots (Pearson's r = 0.858).

2. The shoot mass is positively correlated with the root mass (Pearson's r = 0.553)

3. Total mass is strongly positively correlated with shoot mass (Pearson's r = 0.902)
...
```
The species on the left have higher total mass and mass decreased as you move toward species on the far right. 
Plants with larger biomass seem to have larger ranges. 
Grindelia has the highest biomass and Acmispon has the lowest biomass.
...
Well watered plants have higher biomass than drought stressed plants. This makes sense based on the nature of plants.
...
Specific leave area does not seem to really affect total biomass when looking at it visually, but look statistically. 
...
1. Total mass predicted by SLA, water treatment, and species. (all three)
...
```{r}
model4 <- lm(total_g ~ species_name + water_treatment,
             data = drought_exp_clean)
...
model.sel(model0,
          ...
          model4)
```
*delta = 0 for the best model*

Out of the four models we have tried, the best model is the one that includes water treatment and species 
(the fifth model)
...
- reference for `species_name`   *species_nameAcmispon glabel*
...
- SLA slope: **across drought treatments and species, with eahc 1 unit increase in SLA (mm^2/dry mass of leaf), we expect an increase in total mass of -0.00025 WHICH MEANS WITH THE ESTIMATE AND STANDARD ERROR THERE IS NO EFFECT OF SLA ON TOTAL MASS**  
  Eestimate is close to zero and small t and large p 
...
look at well watered row and compare to reference 
...
model_preds # see the predictions as a data frame
```
